...
namespace It.Unina.Dis.Logbus.WebServices
{
	...
	{
		private const string AsmxTemplate = @"<%@ WebService Language=""C#"" Class=""{0}"" %>", GlobalTemplate = @"<%@ Application Inherits=""{0}"" Language=""C#"" %>";
		...
		private string InstallRuntime ()
		{
			try {
				...
				{
					string wsDeclaration = string.Format (AsmxTemplate, typeof(ChannelManagementService).AssemblyQualifiedName);
					
					using (StreamWriter sw = new StreamWriter (File.Create (Path.Combine (_physicalPath, mgmFname)), Encoding.Default))
						sw.Write (wsDeclaration);
				}
				
				{
					string wsDeclaration = string.Format (AsmxTemplate, typeof(ChannelSubscriptionService).AssemblyQualifiedName);
					
					using (StreamWriter sw = new StreamWriter (File.Create (Path.Combine (_physicalPath, subFname)), Encoding.Default))
						sw.Write (wsDeclaration);
				}
				
				{
					string globalDeclaration = string.Format (GlobalTemplate, typeof(LogbusWebApplication).AssemblyQualifiedName);
					using (StreamWriter sw = new StreamWriter (File.Create (Path.Combine (_physicalPath, "Global.asax")), Encoding.Default))
					...
					if (defs != null)
						foreach (WsdlSkeletonDefinition def in defs) {
							if (def.SkeletonType == null)
								throw new LogbusException (string.Format ("Plugin {0} declares empty skeleton type", plugin.Name));
							if (def.SkeletonType.IsAssignableFrom (typeof(System.Web.Services.WebService)))
								throw new LogbusException (string.Format ("Plugin {0} does not declare a valid WSDL skeleton type", plugin.Name));
							...
							if (!Regex.IsMatch (fname, @"^[a-zA-Z0-9_\.\-%]+$", RegexOptions.CultureInvariant))
								throw new LogbusException (string.Format ("Plugin {0} declares invalid WSDL endpoint: {1}", plugin.Name, def.UrlFileName));
							
							string wsDeclaration = string.Format (AsmxTemplate, def.SkeletonType.AssemblyQualifiedName);
							
							using (StreamWriter sw = new StreamWriter (File.Create (Path.Combine (_physicalPath, fname + ".asmx")), Encoding.Default))
								sw.Write (wsDeclaration);
